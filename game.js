(() => {
  ...
  function toast(m){ state.toastT=2.2; const el=document.getElementById("ui-toast"); el.textContent=m; el.classList.add("show"); }
  ...
  function makeWorld(){
    ...
    state.animals=[
      ["chicken",2.2,3.4,0.08],["chicken",1.6,4.0,0.08],["chicken",2.8,3.1,0.08],
      ["cow",-0.4,3.6,0.13],["sheep",0.8,4.6,0.10],["sheep",1.3,4.9,0.10],
      ["dog",3.4,4.2,0.10],["duck",5.4,2.8,0.08],["duck",5.8,3.1,0.08],
      ["fox",-1.2,1.2,0.08],["rabbit",-0.6,1.8,0.06]
    ].map(([kind,c,r,hf])=>{
      const p=iso(c,r);
  ...
  function sprite(name,x,y,h,face=1){
    ...
    ctx.save(); ctx.translate(x,y); ctx.scale(face,1); ctx.drawImage(im,-w/2,-h,w,h); ctx.restore();
  }
  function walkSprite(kind,x,y,h,face,frame,moving){
    const sheet=imgs[WALKS[kind]]; if(!sheet) return;
  ...
  function draw(){
    ...
    [...state.plots].sort((a,b)=>(a.c+a.r)-(b.c+b.r)).forEach(p=>{
      ...
      if(imgs[img]) ctx.drawImage(imgs[img], s.x-s.tw*0.5, s.y-s.th*0.35+bounce, s.tw, s.th);
      if(p.locked){
        ctx.fillStyle="rgba(30,40,10,.32)";
        ctx.beginPath();
        ctx.moveTo(s.x, s.y-s.th*0.18); ctx.lineTo(s.x+s.tw*0.38, s.y+s.th*0.12);
        ctx.lineTo(s.x, s.y+s.th*0.42); ctx.lineTo(s.x-s.tw*0.38, s.y+s.th*0.12);
        ctx.closePath(); ctx.fill();
        ctx.fillStyle="#fff"; ctx.textAlign="center"; ctx.fillText("荒", s.x, s.y+s.th*0.16);
      }
    });
    const pond=iso(6.2,3.4);
    if(imgs["iso_pond.jpg"]) ctx.drawImage(imgs["iso_pond.jpg"], pond.x-pond.tw*0.7, pond.y-pond.th*0.3, pond.tw*1.4, pond.th*1.3);
    [...state.animals].sort((a,b)=>a.y-b.y).forEach(a=>{
      if(WALKS[a.kind]) walkSprite(a.kind, a.x, a.y, state.h*a.hf, a.face, a.frame, a.moving);
      else sprite(a.kind+".jpg", a.x, a.y, state.h*a.hf, a.face);
    });
